...
public class DashboardInfoGroupOperation: PSOperations.GroupOperation {
  ...
  public init(baseURL: URL, handler: @escaping (DashboardInfo?) -> Void) {
    ...
    let studyQueueOperation = StudyQueueGroupOperation(baseURL: baseURL) { (studyQueue, _) in
      self.studyQueue = studyQueue
    }
    let progressionOperation = LevelProgressionGroupOperation(baseURL: baseURL) { (progression, _) in
    ...
    let srsOperation = SRSDistributionGroupOperation(baseURL: baseURL) { (srs, _) in
      self.srs = srs
    }
